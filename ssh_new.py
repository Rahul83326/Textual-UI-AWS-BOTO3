...
class AwsStatusApp(App):
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "show-all-button":
            ...
            instance_id = event.button.id.split("-")[1]
            await self.connect_via_ssh(instance_id)
    ...
    async def connect_via_ssh(self, instance_identifier: str):
        try:
            pem_file_path = "/Users/vgts/Desktop/AWS_UI/Test-lightsail.pem"

            if not os.path.exists(pem_file_path):
                self.notify("Private key file not found.")
                return
            ...
            print("DEBUG: Available instances with their IPs:")
            for inst_id, name, state, ip, tags in self.instances:
                print(f"DEBUG: Instance {name}, ID: {inst_id}, IP: {ip}")

            for inst_id, name, state, ip, tags in self.instances:
                print(f"DEBUG: Checking instance {inst_id}, name {name}, public IP {ip}")
                if instance_identifier in {inst_id, name}:
                    public_ip = ip
                    instance_id = inst_id
                    if inst_id.startswith("i-"):
                        instance_type = "ec2"
                    elif inst_id in self.LIGHTSAIL_INSTANCES:  
            ...
            if not public_ip:
                if instance_type == "ec2":
                    ...
                    response = lightsail_client.get_instance_public_ports(instanceName=instance_id)
                    if "publicIp" in response["instance"]:
                        public_ip = response["instance"]["publicIp"]

            if not public_ip:
                self.notify(f"No public IP found for instance '{instance_identifier}'.")
                print(f"DEBUG: No public IP found for instance '{instance_identifier}'.")
                return

            print(f"DEBUG: Found public IP: {public_ip} for {instance_identifier}.")

            ssh_user = "ubuntu" if instance_type == "ec2" else "ec2-user"

            ssh_command = f'ssh -i "{pem_file_path}" {ssh_user}@{public_ip}'

            self.notify(f"Opening SSH connection to {instance_type.capitalize()} instance {instance_id} ({public_ip}) as {ssh_user}...")
            subprocess.run(["osascript", "-e", f'tell application "Terminal" to do script "{ssh_command}"'])

        except Exception as e:
            self.notify(f"Error connecting to instance {instance_identifier}: {str(e)}")
            print(f"DEBUG: Error connecting to instance {instance_identifier}: {str(e)}")
